...
def plot_fig(q_pred, net, filename, drone_time_index, color_list):
    num_drone = net.dim // 2
    x = np.linspace(-5,5,100)
    y = np.linspace(-5,5,100)
    xx, yy = np.meshgrid(x, y)
...
def main():
    ...
    #parser.add_argument('--problem', type = str, default = 'door2', help = 'room or maze or door')
    ...
    output_foldername = 'saved_figures/test1/'
    ...
    if args.plotps:
        ...
        x_init_ps = data.y_test['bd'][0,0]
        x_term_ps = data.y_test['bd'][0,1]
        PSsolver = PSmethod(time_endpts, num_nodes, net.dim, net, x_init_ps, x_term_ps, 0)
        PSsolver.solve(PSiters)
        q_pred = PSsolver.get_x()
    else:
        q_pred = net.predict_q(torch.tensor(data.X_test['interval'], dtype = net.dtype, device= net.device), True)[0,...]    
    ...
    num_time_slot = 4
    total_time_ind = q_pred.shape[0]
    for i in range(num_time_slot): 
        drone_time_index = (total_time_ind -1) * i // (num_time_slot-1)
        plot_fig(q_pred, net, output_foldername+ lossname + '_timeindex_{}'.format(i), drone_time_index, color_list=['red', 'black', 'blue', 'green'])
