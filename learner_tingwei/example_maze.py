...
class MZNN(SPNN):
    ...
    # used to output the constraint: dist - const (instead of distsquare - const^2)
    def dist_np(self, q): 
        # compute dist to obs
        ws = np.array(self.ws)
        ql = np.array(self.ql)
        ...
        l2 = ql[None,:,None] ** 2
        q2 = q.reshape([-1, 1, 2])
        d = np.sum((q2 - vs[None,...]) * (ws - vs), axis = -1, keepdims = True) / l2
        t = np.maximum(np.zeros_like(d), np.minimum(np.ones_like(d), d))
        projection = vs[:,None,:] + np.transpose(t, (1, 0, 2)) @ (ws - vs)[:,None,:]
        d = np.sum((q2.squeeze() - projection) ** 2, axis = -1, keepdims = True)
        d = d.reshape([self.num_obs, -1, self.dim // 2, 1])
        h = np.sqrt(np.transpose(d, (0,2,1,3)).reshape([self.num_obs * self.dim // 2, -1, 1])) - (self.qr + self.dr)
        # compute dist between drones
        x = q.reshape([-1, self.dim // 2, 2, 1])
        x = np.transpose(x, (0,2,1,3))
        y = np.transpose(x, (0,1,3,2))
        z = np.sum((x - y)**2, axis = 1)
        mask = ~np.eye(z.shape[1],z.shape[2], dtype=bool)
        min_value = np.sqrt(np.transpose(z[:, mask])[...,None]) - (self.dr * 2)
        h = np.concatenate([min_value, h], axis = 0)
        return h
...
# q and v have shape traj_count * num * dim
# t has shape num (1-dim array)
def compute_cost_hmin_dist(t, net, q, v):
    # compute minimal distance constraint
    dist_h = net.dist_np(q)
    distsq_h = net.h_np(q)
    dist_hmin = np.amin(dist_h)
    distsq_hmin = np.amin(distsq_h)
    # compute cost
    dt = (t[-1] - t[0]) / (t.size - 1)  # a scalar
    L = np.sum(v**2/2, axis=-1)  # trajs * num
    L[:,0] = L[:,0]/2
    L[:,-1] = L[:,-1]/2
    cost = np.mean(np.sum(L, axis = -1) * dt)
    print('cost value: {}\t'.format(cost))
    print('min dist: {}, min distsquare: {}\n'.format(dist_hmin, distsq_hmin), flush=True)
...
def main():
    ...
    parser.add_argument('--testcase', type = int, default = 1, help = '1 for comparison, 2 for mul traj')
    ...
    # print cost and hmin
    print('\nNN cost and hdistmin:')
    q_nn = net_plot.predict_q(data.X_test['interval'], True)[0:traj_count]
    v_nn = net_plot.predict_v(data.X_test['interval'], True)[0:traj_count]
    compute_cost_hmin_dist(data.X_test['interval'].detach().cpu().numpy(), net, q_nn, v_nn) # only print values for the first traj_count many trajs
    ...
    v_ps = np.zeros((traj_count, (num_times-1) * num_nodes, dim))
    # plot initial x
    for i in range(traj_count):
        ...
        v_ps[i] = PSsolver.get_v()
        print(' \n')
        end = perf_counter()
        execution_time = (end - start)
        print('PS running time: {}'.format(execution_time), flush=True)
        print('\nPS cost and hdistmin:')
        t_ps = np.linspace(0, t_terminal, q_ps.shape[1])
        compute_cost_hmin_dist(t_ps, net, q_ps[i:i+1], v_ps[i:i+1]) # only print values for the i-th traj
    if args.testcase == 2:
